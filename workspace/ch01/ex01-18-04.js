// 1. 정보처리 기사 필기 시험의 과목은 소프트웨어 설계, 소프트웨어 개발, 데이터베이스 구축, 프로그래밍 언어 활용, 정보 시스템 구축 관리 입니다.
// 다섯 과목의 점수가 다음과 같을 때 총점과 평균을 출력하는 프로그램을 작성하세요.
// 출력 결과:
// 총점: 303
// 평균: 60.6

let softwareDesign = 48; // 1. 소프트웨어 설계
const softwareDevelopment = 80; // 2. 소프트웨어 개발
const databaseSetup = 50; // 3. 데이터베이스 구축
const programmingUsage = 70; // 4. 프로그래밍 언어 활용
const systemManagement = 55; // 5. 정보 시스템 구축 관리

let sum = softwareDesign + softwareDevelopment + databaseSetup + programmingUsage + systemManagement;
let average = sum /5;

console.log(`총점 : ${sum}`);
console.log(`평균 : ${average}`);

// 2. 최고점과 최저점을 추가로 출력하세요
// 출력 결과:
// 총점: 303
// 평균: 60.6
// 최고점: 80
// 최저점: 48

let max = softwareDesign;
let min = softwareDesign;
if (softwareDevelopment > max){
        max = softwareDevelopment;
}
if (databaseSetup > max) {
        max = databaseSetup;
}
if (programmingUsage > max) {
        max = programmingUsage;
}
if (systemManagement > max) {
        max = systemManagement;
}

// 최저점
if (softwareDevelopment < min){
        min = softwareDevelopment;
}
if (databaseSetup < min) {
        min = databaseSetup;
}
if (programmingUsage < min) {
        min = programmingUsage;
}
if (systemManagement < min) {
        min = systemManagement;
}

console.log(`최고점 : ${max}`);
console.log(`최저점 : ${min}`);






// 3. 과목당 40점 이상, 전과목 평균 60점 이상이면 합격입니다.
// 합격 여부를 추가로 출력하세요.
// 출력 결과:
// 총점: 303
// 평균: 60.6
// 최고점: 80
// 최저점: 48
// 축하합니다. 합격입니다.


if(min > 40 && average >= 60)
{
        console.log("축하합니다. 합격입니다.")
}



// 4. 만약 불합격일 경우 불합격 사유를 추가로 출력하세요.
// 출력 결과 1: softwareDesign = 40일 경우
// 총점: 295
// 평균: 59
// 최고점: 80
// 최저점: 40
// 아쉽지만 불합격입니다. 사유: 평균 60점 미달
console.log("\n");
softwareDesign = 40;
sum = softwareDesign + softwareDevelopment + databaseSetup + programmingUsage + systemManagement;
average = sum/5;
console.log("4-1 문제 출력결과");
console.log(`총점 : ${sum}`);
console.log(`평균 : ${average}`);

max = softwareDesign;
min = softwareDesign;

if (softwareDevelopment > max){
        max = softwareDevelopment;
}
if (databaseSetup > max) {
        max = databaseSetup;
}
if (programmingUsage > max) {
        max = programmingUsage;
}
if (systemManagement > max) {
        max = systemManagement;
}

// 최저점
if (softwareDevelopment < min){
        min = softwareDevelopment;
}
if (databaseSetup < min) {
        min = databaseSetup;
}
if (programmingUsage < min) {
        min = programmingUsage;
}
if (systemManagement < min) {
        min = systemManagement;
}

console.log(`최고점 : ${max}`);
console.log(`최저점 : ${min}`);
if (average < 60){
        console.log("아쉽지만 불합격입니다. 사유: 평균 60점 미달");
}

// 출력 결과 2: softwareDesign = 30일 경우;
// 총점: 285
// 평균: 57
// 최고점: 80
// 최저점: 30
// 아쉽지만 불합격입니다. 사유: 평균 60점 미달(57점)
// 아쉽지만 불합격입니다. 사유: 과락 40점 미달(30점)

//  softwareDesign = 40 일 경우
console.log("\n");
console.log("4-2 출력결과");
softwareDesign = 30;
sum = softwareDesign + softwareDevelopment + databaseSetup + programmingUsage + systemManagement;
average = sum/5;
console.log(`총점 : ${sum}`);
console.log(`평균 : ${average}`);

max = softwareDesign;
min = softwareDesign;

if (softwareDevelopment > max){
        max = softwareDevelopment;
}
else if (databaseSetup > max) {
        max = databaseSetup;
}
else if (programmingUsage > max) {
        max = programmingUsage;
}
else if (systemManagement > max) {
        max = systemManagement;
}

// 최저점
if (softwareDevelopment < min){
        min = softwareDevelopment;
}
if (databaseSetup < min) {
        min = databaseSetup;
}
if (programmingUsage < min) {
        min = programmingUsage;
}
if (systemManagement < min) {
        min = systemManagement;
}

console.log(`최고점 : ${max}`);
console.log(`최저점 : ${min}`);

if ( average < 60)
{
        console.log("아쉽지만 불합격입니다. 사유: 평균 60점 미달(57점)")
}

if ( min < 40)
{
        console.log("아쉽지만 불합격입니다. 사유: 과락 40점 미달(30점)")
}
